...
/**
 * @author qianlong
 * @description 中转站新增/修改事件
 * @Date 2020/11/5 8:18 下午
 **/
@Component
public class NewOrUpdateTransStationEvent extends BaseDbEventStrategy {
    ...
    public String getSupportEventType() {
        return DbEventConstant.EventType.NEW_OR_UPDATE_TRANS_STATION_EVENT;
    }
    ...
    public void strategy(BaseDbEventDTO baseDbEventDTO) {
        ...
        Long stationId = baseDbEventDTO.getEventObject() == null ? null : (Long) baseDbEventDTO.getEventObject();
        if (stationId != null && stationId != 0L) {
            //删除缓存中数据
            FacilityCache.delFacilityCountToday(baseDbEventDTO.getTenantId());

            polymerizationService.addOrUpdateFacility(String.valueOf(stationId), Integer.valueOf(FacilityConstant.FacilityType.TRANSFER_STATION));
        }
