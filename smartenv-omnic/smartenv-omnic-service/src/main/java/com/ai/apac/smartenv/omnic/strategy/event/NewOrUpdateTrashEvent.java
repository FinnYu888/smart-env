...
/**
 * @author qianlong
 * @description 垃圾桶新增/修改事件
 * @Date 2020/11/5 8:18 下午
 **/
@Component
public class NewOrUpdateTrashEvent extends BaseDbEventStrategy {
    ...
    public String getSupportEventType() {
        return DbEventConstant.EventType.NEW_OR_UPDATE_TRASH_INFO_EVENT;
    }
    ...
    public void strategy(BaseDbEventDTO baseDbEventDTO) {
        ...
        Long trashId = baseDbEventDTO.getEventObject() == null ? null : (Long) baseDbEventDTO.getEventObject();
        if (trashId != null && trashId != 0L) {
            //删除缓存中数据
            AshcanCache.delAshcan(trashId);
            AshcanCache.delAshcanCount(baseDbEventDTO.getTenantId());

            polymerizationService.addOrUpdateFacility(String.valueOf(trashId), Integer.valueOf(FacilityConstant.FacilityType.ASHCAN));
        }
