 ...
 *
 * @ClassName: AssessEvent
 * @Description: 垃圾转运信息变化
 * @version: v1.0.0
 * @author: panfeng
 * @date: 2020/10/28
 ...
 * --------------------------------------------------------------------*
 * 2020/10/28  11:39    panfeng          v1.0.0             修改原因
 */
@Component
public class GarbageEvent extends BaseWebsocketPushStrategy {
    ...
    public String getSupportEventType() {
        return WsMonitorEventConstant.EventType.GARBAGE_EVENT;
    }

    @Override
    public void strategy(BaseWsMonitorEventDTO baseWsMonitorEventDTO) {
        streageService.handleBigScreenLastGarbageAmountByRegion(baseWsMonitorEventDTO);
        streageService.handleBigScreenLastGarbageList(baseWsMonitorEventDTO);
        streageService.handleHomeLastGarbageList(baseWsMonitorEventDTO);
    }
